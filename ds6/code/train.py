#!/usr/bin/env python3
"""
Classify AIRRML Dataset 6 positive and negative samples using Her2Neu TCR sequence abundance.
Uses Her2Neu CDR3 sequences as features to predict AIRRML labels.
Similar to the SARSCoV2 classifier for Dataset 3.
"""
...
# External reference data: Parse Bioscience HER2/neu TCR sequences
PARSE_FILE = Path(os.environ.get('PARSE_HER2_FILE', '../reference/tcr_with_antigens_tagged.tsv'))
TRAIN_DATASET_DIR = Path(os.environ.get('DS6_TRAIN_DATA', '../input'))
...
print("=" * 80)
print("AIRRML DATASET 6 CLASSIFICATION USING Her2Neu TCR SEQUENCE ABUNDANCE")
print("=" * 80)
print()

# Step 1: Extract Her2Neu CDR3 sequences
print("Step 1: Extracting Her2Neu CDR3 sequences from Parse dataset...")
her2neu_cdr3s = set()
her2neu_counts = Counter()

chunk_size = 100000
for chunk in pd.read_csv(PARSE_FILE, sep='\t', chunksize=chunk_size,
                         usecols=['cdr3_aa', 'sample', 'is_positive'],
                         low_memory=False):
    her2neu_chunk = chunk[(chunk['sample'] == 'Her2Neu') & 
                          (chunk['is_positive'] == True) & 
                          (chunk['cdr3_aa'].notna())]
    if len(her2neu_chunk) > 0:
        her2neu_cdr3s.update(her2neu_chunk['cdr3_aa'].unique())
        her2neu_counts.update(her2neu_chunk['cdr3_aa'])

her2neu_cdr3s = sorted(list(her2neu_cdr3s))
print(f"  Found {len(her2neu_cdr3s):,} unique Her2Neu CDR3 sequences")
print(f"  Total Her2Neu sequence occurrences: {sum(her2neu_counts.values()):,}")
print()

# Step 2: Load AIRRML metadata
print("Step 2: Loading AIRRML Dataset 6 metadata...")
...
print("Step 3: Building feature matrix (counting Her2Neu CDR3s in each repertoire)...")
...
for idx, row in metadata.iterrows():
    ...
    try:
        ...
        # Count occurrences of each Her2Neu CDR3 in this repertoire
...
print(f"  Total Her2Neu CDR3 occurrences across all repertoires: {X.sum():,}")
print(f"  Repertoires with at least one Her2Neu CDR3: {np.sum(X.sum(axis=1) > 0):,}")
print()
...
print("Top 20 Most Important Her2Neu CDR3 Sequences (Random Forest):")
print("-" * 80)
print(f"{'Rank':<6} {'CDR3 Sequence':<40} {'Importance':>15} {'Train Occurrences':>20}")
print("-" * 80)
for i, idx in enumerate(top_indices, 1):
    cdr3 = her2neu_cdr3s[idx]
    importance = feature_importance[idx]
    # Use only training data for reporting (no data leakage)
    occurrences = X_train[:, idx].sum()
    print(f"{i:<6} {cdr3:<40} {importance:>15.6f} {occurrences:>20,}")

# Save top features (using training data only)
top_features_df = pd.DataFrame({
    'cdr3': [her2neu_cdr3s[idx] for idx in top_indices],
    'importance': [feature_importance[idx] for idx in top_indices],
    'train_occurrences': [X_train[:, idx].sum() for idx in top_indices]
})
top_features_df.to_csv(OUTPUT_DIR / 'top_her2neu_features.csv', index=False)
...
with open(OUTPUT_DIR / 'her2neu_cdr3s.pkl', 'wb') as f:
    pickle.dump(her2neu_cdr3s, f)
...
print("  - top_her2neu_features.csv")
print("  - logistic_regression_model.pkl")
print("  - random_forest_model.pkl")
print("  - xgboost_model.pkl")
print("  - scaler.pkl")
print("  - her2neu_cdr3s.pkl (feature list)")
print()
print("=" * 80)
