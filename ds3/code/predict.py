"""
Generate predictions for test repertoires and ranked sequence list for DS3.

This script:
1. Loads the trained DS3 Logistic Regression model (SARS-CoV-2 CDR3-based)
2. Extracts features from TEST repertoires (counts of SARS-CoV-2 CDR3 sequences)
...
TRAIN_DATA_DIR = Path(os.environ.get('DS3_TRAIN_DATA', '../input/train'))
TRAIN_METADATA_PATH = TRAIN_DATA_DIR / "metadata.csv"
TEST_DATA_DIR = Path(os.environ.get('DS3_TEST_DATA', '../input/test'))
...
class DS3Predictor:
    """Predictor for DS3 using SARS-CoV-2 CDR3 sequence counts."""
    ...
    def __init__(self, verbose: bool = True):
        ...
        self.model = None
        self.scaler = None
        self.sarscov2_cdr3s = None
        self.results_metadata = None
        
    def load_model(self):
        """Load the trained Logistic Regression model, scaler, and SARS-CoV-2 CDR3 list."""
        if self.verbose:
        ...
        # Load SARS-CoV-2 CDR3 sequences (feature vocabulary)
        with open(MODEL_DIR / 'sarscov2_cdr3s.pkl', 'rb') as f:
            self.sarscov2_cdr3s = pickle.load(f)
        ...
        if self.verbose:
            print_flush(f"Model loaded. SARS-CoV-2 CDR3s: {len(self.sarscov2_cdr3s)}")
    ...
    def extract_features(self, repertoires: Dict[str, pd.DataFrame], 
        ...
        """Extract features by counting SARS-CoV-2 CDR3 sequences in each repertoire."""
        if self.verbose:
            print_flush("\nExtracting features (counting SARS-CoV-2 CDR3s)...")
        ...
        for sample_id in tqdm(sample_ids, disable=not self.verbose):
            ...
            # Count occurrences of each SARS-CoV-2 CDR3 in this repertoire
            rep_cdr3_counts = Counter(tcr_df['junction_aa'].dropna())
            feature_vector = [rep_cdr3_counts.get(cdr3, 0) for cdr3 in self.sarscov2_cdr3s]
    ...
    def predict_test_repertoires(self):
        ...
        output_path = OUTPUT_DIR / "ds3_test_predictions.csv"
    ...
    def rank_training_sequences(self, top_n: int = 50000):
        ...
        # Create mapping from SARS-CoV-2 CDR3 to importance
        cdr3_to_importance = {cdr3: imp for cdr3, imp in zip(self.sarscov2_cdr3s, feature_importance)}
        ...
        # Create set of SARS-CoV-2 CDR3s for fast lookup
        sarscov2_set = set(self.sarscov2_cdr3s)
        ...
        for sample_id, tcr_df in tqdm(repertoires.items(), disable=not self.verbose):
            ...
            for _, row in tcr_df.iterrows():
                ...
                # Check if this sequence is a SARS-CoV-2 CDR3
                if junction_aa in sarscov2_set:
                    sequence_data[junction_aa]['sarscov2_count'] += 1
        ...
        for junction_aa, data in tqdm(sequence_data.items(), disable=not self.verbose):
            # SARS-CoV-2 CDR3 importance score
            if junction_aa in cdr3_to_importance:
                sarscov2_score = cdr3_to_importance[junction_aa]
            else:
                sarscov2_score = 0.0
            ...
            # SARS-CoV-2 association (whether it's a SARS-CoV-2 CDR3)
            is_sarscov2 = 1.0 if junction_aa in sarscov2_set else 0.0
            
            # Combined importance score
            # Weight: SARS-CoV-2 CDR3 importance (most important), positive ratio, and SARS-CoV-2 association
            importance_score = (
                0.6 * sarscov2_score +  # SARS-CoV-2 CDR3 importance (most important)
                0.3 * pos_ratio +  # Positive sample association
                0.1 * is_sarscov2  # SARS-CoV-2 association bonus
            )
            ...
            scored_sequences.append({
                ...
                'sarscov2_score': sarscov2_score,
                'pos_ratio': pos_ratio,
                'is_sarscov2': is_sarscov2,
        ...
        output_path = OUTPUT_DIR / "ds3_ranked_sequences.csv"
    ...
    def create_submission_file(self, test_predictions: pd.DataFrame, ranked_sequences: pd.DataFrame):
        ...
        for idx, (_, row) in enumerate(ranked_sequences.iterrows(), 1):
            train_entries.append({
                'ID': f'train_dataset_3_seq_top_{idx}',
        ...
        output_path = OUTPUT_DIR / "ds3_submission.csv"
        ...
        if self.verbose:
            ...
            print_flush(submission_df[submission_df['dataset'] == 'test_dataset_3'].head(3).to_string())
            print_flush(f"\nFirst few training sequence entries:")
            print_flush(submission_df[submission_df['dataset'] == 'train_dataset_3'].head(3).to_string())
...
def main():
    """Main function."""
    print_flush("="*70)
    print_flush("DS3 PREDICTIONS AND SEQUENCE RANKING")
    print_flush("="*70)
    
    # Initialize predictor
    predictor = DS3Predictor(verbose=True)
    ...
    print_flush("="*70)
    print_flush(f"Test predictions: {OUTPUT_DIR / 'ds3_test_predictions.csv'}")
    print_flush(f"Ranked sequences: {OUTPUT_DIR / 'ds3_ranked_sequences.csv'}")
    print_flush(f"Submission file: {OUTPUT_DIR / 'ds3_submission.csv'}")
